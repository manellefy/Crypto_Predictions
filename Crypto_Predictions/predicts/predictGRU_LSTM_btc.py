...
def import_model():
    ...
    if os.path.exists(current_week_folder_path):
        model_path = os.path.join(current_week_folder_path, 'model_gru_lstm_btc.h5')
    else:
        model_path = os.path.join(existing_models_folder_path, 'model_gru_lstm_btc.h5')
    # Importer le modèle
...
def predict_future_value(model, future_date, max_date):
    ...
    bnb_data_path = os.path.join(data_dir, recent_folder, 'BTC_data.csv')
