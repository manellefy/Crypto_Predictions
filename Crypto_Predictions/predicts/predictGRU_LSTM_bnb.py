...
def import_model():
    ...
    if os.path.exists(current_week_folder_path):
        model_path = os.path.join(current_week_folder_path, 'model_gru_lstm_bnb.h5')
    else:
        model_path = os.path.join(existing_models_folder_path, 'model_gru_lstm_bnb.h5')
    # Importer le modèle
...
def predict_future_value(model, future_date, max_date):
    ...
    bnb_data_path = os.path.join(data_dir, recent_folder, 'BNB_data.csv')
...
if __name__ == '__main__':
    ...
    future_date = datetime(2024, 4, 30, 19, 55, 0)
    max_date = datetime(2024, 4, 24,0,0,0)
